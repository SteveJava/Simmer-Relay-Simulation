...
public class MedleySimulation {
	...
	public static void setupGUI(int frameX, int frameY) throws InterruptedException {
		...
		// Initialize all necessary objects before the GUI
		finishLine = new FinishCounter(); // counters for people inside and outside the club
		...
		startB.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				new Thread(stadiumView).start();
				new Thread(counterDisplay).start();
				for (int i = 0; i < numTeams; i++) {
					teams[i].start();
				}
			}
