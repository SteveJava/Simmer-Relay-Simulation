...
public class Swimmer extends Thread {
	...
	private CountDownLatch latchStart; // Current latch of swimmer
	private CountDownLatch latchNext; // Latch to be passed to next swimmer
	private CyclicBarrier barrierStart = new CyclicBarrier(10);
	...
	//Constructor
	Swimmer( int ID, SwimTeam t, PeopleLocation loc, FinishCounter f, int speed, SwimStroke s, CountDownLatch latchStart, CountDownLatch latchNext, CyclicBarrier barrierStart) {
		this.swimStroke = s;
		this.ID=ID;
		movingSpeed=speed; //range of speeds for swimmers
		this.myLocation = loc;
		this.team= t;
		start = stadium.returnStartingBlock(team.getID());
		finish=f;
		rand=new Random();
	...
	public void run() {
		try {
			...
			if(swimStroke.order==4) {
				latchNext.countDown();
				finish.finishRace(ID, team.getID()); // fnishline
			}
			else {
